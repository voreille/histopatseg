...
class ProtoNet:
    ...
    def fit(self, X: torch.Tensor, y: torch.Tensor, verbose: bool = True) -> None:
        ...
        if verbose:
            ...
            for cls, count in (
                pd.DataFrame(labels_source.numpy())
                .value_counts()
        ...
        if self.center_feats:
            if verbose:
                print("Applying centering...")
            self.mean = feats_source.mean(
                dim=0, keepdims=True
            )  # [1 x D]-dim vector, average taken over samples
        ...
        assert self.prototype_embeddings.shape == (
            len(self.prototype_labels),
            feats_source.shape[1],
        )
    ...
    def predict(self, X: torch.Tensor) -> torch.Tensor:
        ...
        pw_dist = (feats_query - prototype_embeddings).norm(
            dim=-1, p=2
        )  # [N x C x D] distance/sim matrix
    ...
    def get_topk_queries(
        ...
        """
        dist, topk_inds = self._get_topk_queries_inds(
            X, topk=topk
        ...
        y_nearest = torch.Tensor(
            np.concatenate(
                [
                    ...
                    * topk_inds.shape[1]
                    for i in self.prototype_labels
                ]
            )
        )
    ...
    def _get_topk_prototypes_inds(
        self, X: torch.Tensor, topk: int = 1
